...
const initUsersList = localStorage.getItem('usersList') == null ?
    [{
        userId: 1,
        fName: 'Behrouz',
        lName: 'Beik',
        role: 'Admin',
        email: 'beik.behrouz@gmail.com',
        password: '123456',
        profileImage: 'https://media.licdn.com/dms/image/C5603AQEPvbFlJW75sw/profile-displayphoto-shrink_200_200/0/1542182391599?e=1692835200&v=beta&t=z5LEfnZ1UJEfrVUqKRafA5kU8e2DxQCnMMgxWDQeznM',
        callNumber: '9372935586',
        activeFlag: true
    },
    {
        userId: 2,
        ...
        lName: 'Beik2',
        role: 'user',
        email: 'beik2.behrouz2@gmail.com',
        password: '123456',
...
const initUserCount = localStorage.getItem('userCount') == null ? 2 : 
    JSON.parse(localStorage.getItem('userCount'))

const initActiveUsers = localStorage.getItem('activeUsers') == null ? 2 : 
    JSON.parse(localStorage.getItem('activeUsers'))

const initIsauth = localStorage.getItem('isauth') == null ? false : 
    JSON.parse(localStorage.getItem('isauth'))

const initToken = localStorage.getItem('token') == null ? null : 
    JSON.parse(localStorage.getItem('token'))

const initialState = {
    usersList: initUsersList,
    userCount: initUserCount,
    activeUsers: initActiveUsers,
    isauth:initIsauth,
    token:initToken
}
    ...
    reducers: {
        addUser: (state, action) => {
            ...
            localStorage.setItem(
                'usersList',
                JSON.stringify(state.usersList.map((item)=>item))
            )
            localStorage.setItem("userCount", JSON.stringify(state.userCount))
            localStorage.setItem("activeUsers", JSON.stringify(state.activeUsers))
        },
        ...
        authenticateUser: (state,action) => {
            const auth=state.usersList.map((user) => {
